...
function AdminSignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/admins/accept-invite/`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setError(data.detail || "Failed to activate account. Your invitation link may be expired.");
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="rounded-[2.5rem] border border-primary/20 bg-card p-8 md:p-10 shadow-gold backdrop-blur-sm">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          Please set a secure password to activate your administrator account.
        </p>
      </div>
